"""Repeated evaluation for the trained dynamic DQN baseline."""
...
DEFAULT_ENV_CONFIG_PATH = Path("configs/env/dynamic_grid_2d.yaml")
DEFAULT_TRAINING_CONFIG_PATH = Path("configs/training/dqn_dynamic_full.yaml")
DEFAULT_OUTPUT_PATH = Path("results/reports/m10/dynamic_dqn_repeated_eval.json")
DEFAULT_REPEATS = 5
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    """Parse repeated dynamic DQN evaluation arguments."""
    parser = argparse.ArgumentParser(
        description="Repeated evaluation for the trained dynamic DQN baseline.",
    )
...
def main() -> None:
    """Run repeated deterministic evaluation for dynamic DQN."""
    args = parse_args()
    _validate_args(args)
    ...
    print("Repeated dynamic DQN evaluation complete.")
